...
from dense_gru import Dense_GRU
...
def data_stft(data): # B,seq_length,3000
    ...
    freqs, time, Zxx = stft(data,fs=1,window='hamming',nperseg=256,nfft=None)
    Zxx = np.abs(Zxx).transpose(0, 2, 1)
    return Zxx.reshape(-1,seq_length,Zxx.shape[1],Zxx.shape[2])   # B,seq_length,25,129

def batch_data(x, y, batch_size):
    # 加载数据并打乱，使用 yield 迭代器迭代出每个batch
...
def main(args=None):
    ...
    tf.app.flags.DEFINE_string('output_dir', 'outputs_2013/outputs_eeg_fpz_cz',
                               ...
                               """and outputs.""")
    tf.app.flags.DEFINE_integer('num_folds', 2,
    ...
    hparams = tf.contrib.training.HParams(
        ...
        max_time_step=3,  # 5 3 second best 10# 40 # 100
        output_max_length=3 + 2,  # max_time_step +1
        akara2017=True,
